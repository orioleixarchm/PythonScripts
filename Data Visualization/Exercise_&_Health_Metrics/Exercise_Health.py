...
#PCA and Cluster composition
centroid = pd.DataFrame(cluster.cluster_centers_, columns=continuous_data.drop(columns='Cluster').columns)
PCA_Composition = pd.DataFrame(pca.components_.T, columns=['PCA I', 'PCA II'], index=continuous_data.drop(columns='Cluster').columns)
print('Cluster Centroids Scaled Mean Characteristics:')
print(centroid,'\n')
print('Cluster Unscaled Data Mean Characteristics:')
print(continuous_data.groupby('Cluster').mean(),'\n')
print('Composition of the Principal Components:')
print(PCA_Composition,'\n')

fig_comp, axes_comp = plt.subplots(1,2)
fig_comp.suptitle('Principal Component Analysis decomposition')
fig_comp.tight_layout(pad=2.5)
positive = mpatches.Patch(color='Blue', label='Positive Contribution')
negative = mpatches.Patch(color='Orange', label='Negative Contribution')
PCA_Composition['PCA I'].sort_values().abs().plot(kind='barh', color=['orange' if val_I <= 0 else 'blue' for val_I in PCA_Composition['PCA I'].sort_values()], ax=axes_comp[0])
axes_comp[0].set_title('Principal Component I')
axes_comp[0].set_xlabel('Contribution magnitude')
axes_comp[0].legend(handles=[positive, negative], loc='best')
PCA_Composition['PCA II'].sort_values().abs().plot(kind='barh', color=['orange' if val_II <= 0 else 'blue' for val_II in PCA_Composition['PCA II'].sort_values()], ax=axes_comp[1])
axes_comp[1].set_title('Principal Component II')
axes_comp[1].set_xlabel('Contribution magnitude')
axes_comp[1].legend(handles=[positive, negative], loc='best')
